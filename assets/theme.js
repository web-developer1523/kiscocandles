...
function updatePrices (container, variant, productTemplate) {
  ...
  if (variant.available) {
    ...
    var source = document.getElementById("product-application-json").innerHTML;
    var sourceObject = JSON.parse(source);
    var objectOffers = sourceObject['offers'];

    // Get count and price from the 'thing' object
    var variableCount = sourceObject['thing'] ? sourceObject['thing']['product_count'] : null;
    var product_price = sourceObject['thing'] ? sourceObject['thing']['product_price'] : null;

    // Check if variableCount is empty, undefined, or null
    if(!variableCount && document.querySelector('.pr-grid-cnt-'+variant.id)){
      variableCount = parseInt(document.querySelector('.pr-grid-cnt-'+variant.id).value);
      product_price = document.querySelector('.pr-grid-price-'+variant.id).value;
    }

    // Loop through offers to find matching variant
    for (var i = 0; i < objectOffers.length; i++) {
      // Check if the offer URL contains the variant ID
      if(objectOffers[i]['url'] && objectOffers[i]['url'].includes('variant=' + variant.id)){
        if(objectOffers[i]['count']) variableCount = objectOffers[i]['count'];
        if(objectOffers[i]['price']) product_price = objectOffers[i]['price'];
        break;
      }
    }
    console.log('Count:', variableCount, 'Price:', product_price, 'Variant ID:', variant.id);

    // Safety checks and calculation
    const spackElement = document.getElementById("spack-pricing");
    if (spackElement) {
      if (variableCount && product_price) {
        const count = parseInt(variableCount);
        const priceValue = parseFloat(product_price);
        
        if (!isNaN(priceValue) && !isNaN(count) && count > 0) {
          const priceInCents = priceValue > 1000 ? priceValue : priceValue * 100;
          
          
          var packPriceInCents = Math.floor(priceInCents / count);
          var packPrice = packPriceInCents / 100;
          
          spackElement.innerHTML = '$' + packPrice.toFixed(2) + ' ea.';
        } else {
          spackElement.innerHTML = '';
        }
      } else {
        spackElement.innerHTML = '';
      }
...
class Product {
    ...
    if (this.formElement) {
      ...
      product(data => {
        ...
        this.productForm = ProductForm(this.container, this.formElement, data, {
          onOptionChange: e => this.onOptionChange(e,data.id,data.title),
  ...
  onOptionChange(_ref2, productId,productTitle) {
    let {
      dataset: {
        variant
      },
    ...
    if(variant.option1 !== null){
      variantOpt = variant.option1;
    }
    if(variant.option2 !== null){
      variantOpt = variantOpt + ' - '+variant.option2;
    }
    if(variant.option3 !== null){
      variantOpt = variantOpt + ' - '+variant.option3;
    }
    var newTitle = productTitle + ' - '+ variantOpt;
    document.querySelector('.pdp-item-title-ele').innerHTML = newTitle;
...
function QuickSearchDesk (node, header) {
  const overlay = n$2("[data-overlay-desk]", node);
  const form = n$2("[data-quick-search-form-desk]", node);
  const input = n$2("[data-input-desk]", node);
  const clear = n$2("[data-clear-desk]", node);
  const resultsContainer = n$2("[data-results-desk]", node);
  const predictiveSearch = PredictiveSearch(resultsContainer);
  const closeButton = n$2("[data-close-icon-desk]", node);
  const searchToggles = t$2("[data-search-desk]", header);
...
function updateAnnouncementHeight() {
  if (document.querySelector('html').classList.contains('sticky-header-enabled')) {
    const stickySetting = document.querySelector('html').getAttribute('data-enable-sticky-announcement-bar');
    ...
    if (stickySetting !== 'never' && announcementWrapper && !announcementWrapper.classList.contains('hidden') && headerWrapper) {
      const announceHeight = announcementWrapper.offsetHeight;
      headerWrapper.style.setProperty('top', announceHeight + 'px');
      document.documentElement.style.setProperty('--announcement-height', announceHeight + 'px');
    } else {
      headerWrapper.style.setProperty('top', '0px');
...
document.addEventListener("visibilitychange", function logData() {
  if (document.visibilityState === "hidden" && navigator.sendBeacon) {
    ...
    if (searchButtons.length > 0) {
      ...
      const quickSearchDesk = QuickSearchDesk(n$2("[data-quick-search-desk]"), this.container);
...
const filterBar = node => {
  ...
  function showDropdown(button) {
    ...
    if (dropdown) {
      if (dropdownToggle === "filter-bar-filters") {
        document.querySelector('.filter-bar__button-text').innerHTML = 'Hide Filters';
        slideStop(dropdown);
  ...
  function hideDropdown(button) {
    ...
    if (dropdown) {
      ...
      if (dropdownToggle === "filter-bar-filters") {
        document.querySelector('.filter-bar__button-text').innerHTML = 'Show Filters';
        slideStop(dropdown);
        slideUp(dropdown);
      } else if (group) {
...
//document.querySelector(".header-custom-search").onclick = function() {
/*document.querySelector(".header-custom-search").addEventListener('focus', function(e) {
  document.querySelector(".header-desktop-trigger-click").click();
});*/

var swatchMoreBtn = document.querySelectorAll(".list-swatch-more-cnt");
swatchMoreBtn.forEach(sbtn => {
  sbtn.addEventListener('click', function() {
    var spid = this.getAttribute('data-spid');
    var swatchList = document.querySelectorAll("."+spid);
    swatchList.forEach((sList) => {
      sList.classList.remove('swatch-d-none');
    });
    this.classList.add('swatch-d-none');
  });
});

var listSwatches = document.querySelectorAll(".list-swatch-opt-ch");
listSwatches.forEach(swatchBtn => {
  swatchBtn.addEventListener('click', function() {
    var spid = this.getAttribute('data-product-id');
    var spImg = this.getAttribute('data-variantimg');
    var spPrice = this.getAttribute('data-variantprice');
    var spPerPack = this.getAttribute('data-perPack');
    document.querySelector('.img-pid-'+spid).setAttribute('src',spImg);
    document.querySelector('.img-pid-'+spid).setAttribute('srcset',spImg);
    document.querySelector('.price-pid-'+spid).innerHTML = spPrice;
    document.querySelector('#list-spack-pricing-'+spid).innerHTML = spPerPack;
  });
});

var tabList = document.querySelectorAll(".tab_content_btn");
if(tabList){
  tabList.forEach(tabBtn => {
    tabBtn.addEventListener('click', function() {
      tabList.forEach(actBtn => {
        actBtn.classList.remove('selected');
      });
      this.classList.add('selected');
      var index = this.getAttribute('data-tab-item');
      var dataContent = document.querySelectorAll(".tab_content_class");
      dataContent.forEach((dbtn) => {
        dbtn.classList.add('hide');
      });
      document.querySelector('.tab_content_data-'+index).classList.remove('hide');
    });
  });
}
...
body.addEventListener('click', function(e) {
  var btnLink = e.target.getAttribute('href');
  var thisEl = e.target.closest('.desk-quick_search');
  if(e.target.classList.contains('areviews_questions') || e.target.parentNode.classList.contains('areviews_questions') || e.target.parentNode.parentNode.classList.contains('areviews_questions')){
    setTimeout(function () {
      console.log('2111121');
    document.querySelector('.Questions_tab').click();
    }, 500);
  }
  else if(btnLink == '#chat_open'){
    e.preventDefault();
    document.querySelector('#ShopifyChat').shadowRoot.querySelector('.chat-toggle').click();
  }
  else if(thisEl == null){
    var formCont = document.querySelector('.desk-quick_search');
    formCont.classList.remove('active');
  }
});

var candleSizeGuide = document.querySelector('#product-candle-size-guide');
if(candleSizeGuide){
  candleSizeGuide.addEventListener('click', function(e) {
    e.preventDefault();
    e.stopPropagation();
    document.querySelector('.size-guide-thumb').click();
  });
}